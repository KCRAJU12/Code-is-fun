...
# Field parameters
#E0_array = 2*np.pi * np.array([0.1,1,2,3,4,5,6,7]) # uncomment this line later
E0_array = 2*np.pi * np.array([0.01,0.1,2,3,4,5,6,7])
#E0_array = 2*np.pi * np.array([5,6,7]) # delete this line later
#for shifts,d in zip(shifts,range(len(Delta_array))):
plt.figure(figsize=(10,10))   # Gives the flexibility to change the size of the figure
labels = ['0.01*Gamma','0.1*Gamma','1*Gamma','2*Gamma','3*Gamma','4*Gamma','5*Gamma']
...
#----------- EMISSION SPECTRUM ------------------------------------------------
    ...
    spec = spectrum(Hrot, wlist-Delta, [spont_emis1,spont_emis2],(a1.dag()+a2.dag()),(a1+a2),solver ='es',use_pinv=False) # Here [spont_emis1,spont_emsi2] is a list of collapsed operator (c_ops) and ai is a_op a2 is b_op
    #fig, ax = plt.subplots()
    plt.semilogy(wlist / (2 * np.pi), spec*10**(i-2))
    plt.xlabel('Emission frequency (GHz)')
    plt.ylabel('Logarithmic Emission intensity')
